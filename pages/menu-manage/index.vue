<template>
          ...
          <q-card>
            <q-card-section>
              <q-form ref="insertForm">
                ...
                />
                <q-input
                  v-model="param.upperMenuSn"
                  outlined
                  stack-label
                  label="상위메뉴ID"
                  :rules="[parentMenuId_rules]"
                />
                <q-input
                  v-model="param.menuSortOrdr"
                  outlined
                  stack-label
                  label="정렬순서"
                  :rules="[number_rules]"
  ...
  <div class="popup">
    <q-dialog v-model="showDetailDialog" @hide="onReset">
      <q-layout container>
        <q-header>
          <q-toolbar class="bg-primary">
        ...
        <q-card-section class="q-pt-none">
          <q-form ref="insertForm">
            <q-input
              v-model="param.menuNm"
              label="메뉴명"
              :rules="[(val: string) => !!val || '메뉴명' + ERROR_FIELD_EMPTY]"
              class="input"
              outlined
            />
            <q-input
              v-model="param.menuUrl"
              label="메뉴URL"
              :rules="[(val: string) => !!val || '메뉴URL' + ERROR_FIELD_EMPTY]"
              class="input"
              outlined
            />
            <q-input
              v-model="param.menuDepth"
              label="메뉴레벨"
              :rules="[(val: string) => !!val || '메뉴레벨' + ERROR_FIELD_EMPTY]"
              class="input"
              outlined
            />
            <q-input
              v-model="param.menuSortOrdr"
              label="정렬순서"
              :rules="[(val: string) => !!val || '정렬순서' + ERROR_FIELD_EMPTY]"
              class="input"
              outlined
            />
            <q-input
...
const useAtSelectedOption = ['전체', 'Y', 'N']
...
const selectAllMenu = async () => {
  const result = await $fetch<ApiResponse<Data[]>>(
    '/playground/public/menu/select-all?page=' +
      (currentPage.value - 1) +
      '&size=' +
      itemsPerPage.value,
    {
...
const onSubmit = async () => {
  try {
    ...
    if (valid) {
      // 저장 API 호출
      await $fetch<ApiResponse<Data[]>>('/playground/public/menu/save', {
        method: 'POST',
        body: JSON.stringify(param.value)
      })
        .then(() => {
...
const onSubmitSelect = async () => {
  ...
  searchParam.value.useYn =
    useYnSelected.value.length == 1 ? useYnSelected.value[0] : ''
