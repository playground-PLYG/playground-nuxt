<template>
    ...
    <q-dialog ref="dialogRef" @hide="">
              ...
              <q-card-actions>
                  ...
                  <q-btn flat round icon="event" :disable="!insertMode">
                    <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                      <q-date v-model="registDateTime.beginDate" :options="beginDateLimitFn">
                        <div class="row items-center justify-end q-gutter-sm">
                          <q-btn label="OK" color="primary" flat @click="onClickDate('BEGIN')" v-close-popup />
                  ...
                  <q-btn flat round icon="access_time" :disable="!insertMode">
                    <q-popup-proxy cover transition-show="scale" transition-hide="scale" ref="beginTimeProxy">
                      <q-time v-model="registDateTime.beginTime">
                        <div class="row items-center justify-end q-gutter-sm">
                          <q-btn label="OK" color="primary" flat @click="" v-close-popup />
                          <q-btn label="Cancel" color="primary" flat v-close-popup />
                        </div>
                      </q-time>
                    </q-popup-proxy>
                  ...
                  <q-btn flat color="primary" :disable="!insertMode">
                      투표시작 일시 : {{registDateTime.beginDate + ' ' + registDateTime.beginTime + ':00'}}
                  </q-btn>
                  <div style="margin-left:50px;">
                      <q-checkbox :disable="!insertMode" color="black" v-model="registVoteData.anonymityVoteAlternativeBoo" label="익명투표여부" />
                  </div>
              </q-card-actions>
              <q-card-actions>
                  <q-btn flat round icon="event" :disable="!insertMode">
                    <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                      <q-date v-model="registDateTime.finishData" :options="finishDateLimitFn">
                        <div class="row items-center justify-end q-gutter-sm">
                          <q-btn label="OK" color="primary" flat @click="onClickDate('FINISH')" v-close-popup />
                          <q-btn label="Cancel" color="primary" flat v-close-popup />
                        </div>
                      </q-date>
                    </q-popup-proxy>
                  ...
                  <q-btn flat round icon="access_time" :disable="!insertMode">
                    <q-popup-proxy cover transition-show="scale" transition-hide="scale" ref="finishTimeProxy">
                  ...
                  <q-btn flat color="primary" :disable="!insertMode">
                      투표종료 일시 : {{registDateTime.finishData + ' ' + registDateTime.finishTime + ':00'}}
                  </q-btn>  
                  <div style="margin-left:50px;">
                      <q-select outlined v-model="registVoteData.voteKindName" :options="codeName" label="투표종류" round dense flat class="select" :readonly="!insertMode"/>
                  </div>
              </q-card-actions>
          </q-card>
      </div>
      ...
      <div class="q-pa-md" v-for="(qestn, index) in registVoteData.qestnRequestList" :key="qestn.questionSsno">
        <q-card class="my-card">
          <q-card-section class="bg-light-blue-6 text-white">
              ...
              <div class="q-pl-lg" style="display: inline-flex;">
                <q-btn align="center"  size="xs" v-show="insertMode" :disable="!insertMode" icon="close" outline round color="white" >
          ...
          <q-list>
            <q-item v-for="(iem, index) in qestn.voteIemRequestList" :key="iem.itemId"  tag="label" v-ripple>
              ...
              <q-item-section avatar>
                <q-btn size="xs" :disable="!insertMode" icon="close" outline round color="grey-7">
                  <q-popup-proxy :offset="[200, -20]">
                    <q-banner class="bg-indigo-10 text-white">
                      항목을 삭제하시겠습니까?
                      <q-card-actions align="center">
                        <q-btn push color="white" size="xs" text-color="black" label="삭제" style="margin-top:5px;" 
                          v-close-popup
                          @click="delVoteItem(qestn.questionSsno, iem.itemId)"
                        />
                        <q-btn push color="white" size="xs" text-color="black" label="취소" style="margin-top:5px;" v-close-popup/>
                      </q-card-actions>
                    </q-banner>
            ...
            <q-card-actions align="left">
              ...
              <q-btn size="sm" outline color="indigo-7"  label="항목추가" :disable="!insertMode">
              ...
              <q-btn size="sm" outline color="indigo-7"  label="식당검색" :disable="!insertMode" v-show="registVoteData.voteKindName.indexOf('식사') > -1 ? true : false"/>
            </q-card-actions>
          </q-list>
        </q-card>
      </div>
        ...
        <q-card-actions align="right">
          <q-btn outline rounded color="indigo-7" label="질문추가하기" @click="addQuestion" :disable="!insertMode"/>
        </q-card-actions>
        <q-card-actions align="center">
          <q-btn align="around" class="btn-fixed-width" color="brown-5" :label="insertBtnLabe" icon="lightbulb_outline" @click="setInsertData" />
          <q-dialog v-model="isInsertConfirm" persistent backdrop-filter="blur(4px)">
            <q-card>
...
let registVoteIemData = ref<VoteItemDetailDataType>({
  itemId : '',
  itemName : '',
  questionSsno : 0,
  voteSsno : 0
...
const setInsertData = () => {
  ...
  registVoteData.value.anonymityVoteAlternative = reqVote.anonymityVoteAlternativeBoo ? 'Y' : 'N';

  if(registVoteData.value.voteSubject == '' || registVoteData.value.voteSubject == undefined || registVoteData.value.voteSubject == null){
    alert('투표제목을 입력해주세요.');
    return false;
  }

  // 1. 등록하기 버튼 클릭시 날짜, 시간 잘 들어가는지 확인하고 형식 맞춰서 보내주기
  if(registDateTime.value.beginDate == '' || registDateTime.value.beginDate == undefined || registDateTime.value.beginDate == null
    || registDateTime.value.beginTime == '' || registDateTime.value.beginTime == undefined || registDateTime.value.beginTime == null
    || registDateTime.value.finishData == '' || registDateTime.value.finishData == undefined || registDateTime.value.finishData == null
    || registDateTime.value.finishTime == '' || registDateTime.value.finishTime == undefined || registDateTime.value.finishTime == null
  ){
    alert('투표시작 일시 혹은 투표종료 일시를 선택해주세요.');
    return false;
  }

  let setBeginDate = dateTimeFormatter(registDateTime.value.beginDate, registDateTime.value.beginTime);
  let setEndDate = dateTimeFormatter(registDateTime.value.finishData, registDateTime.value.finishTime);
  ...
  if(reqVote.voteKindName =='' || reqVote.voteKindName == undefined || reqVote.voteKindName == null){
    alert('투표종류를 선택해주세요.')
    return false;
  }else{
    for (const cd of codeDataList) {
      if(cd.codeNm == reqCodeName){
        registVoteData.value.voteKindCode = cd.codeId;
      }
    }
  }
...
const addVoteItem = (questionSsno:number) => {
  let valItemId = addVoteItemData.value.itemId;
  let valItemName = addVoteItemData.value.itemName;
  if(valItemId == '' || valItemId == undefined || valItemName == '' || valItemName == undefined){
...
const delVoteItem = (questionSsno:number, itemId:string) => {
  let qestnList = registVoteData.value.qestnRequestList;
  qestnList.forEach((qes) => {
    if(qes.questionSsno == questionSsno){
      if(qes.voteIemRequestList.length <= 1){
        ...
        qes.voteIemRequestList = qes.voteIemRequestList.filter((iem) => { if(iem.itemId != itemId) return iem })
      }
    }
  })
  registVoteData.value.qestnRequestList = qestnList;
...
const beginDateLimitFn = (date:string) => {    
  ...
  return date <= ((chkFinDate == '' || chkFinDate == undefined || chkFinDate == null) ? '9999/12/31' : chkFinDate );
}

const finishDateLimitFn = (date:string) => {
  let chkStrDate:string = registDateTime.value.beginDate;
  return date >= ((chkStrDate == '' || chkStrDate == undefined || chkStrDate == null) ? '9999/12/31' : chkStrDate );
}
   
const dateTimeFormatter = (date:string, time:string) => {
  return date.toString().replaceAll('/', '-') + ' ' + time.toString()+':00.000'
}
