<template>
      ...
      <div class="q-pa-md" v-for="(qestn, index) in showQuestionData" :key="qestn.questionSsno">
        <q-card class="my-card">
          <q-card-section class="bg-light-blue-6 text-white">
              <!-- <div class="text-h6" style="display: inline-flex; margin-right: 10px;">{{ qestn.questionContents }}</div> -->
  ...
  let afterDateTime = ref<afterDateTimeType>({
    beginDate: '',
    beginTime: '',
    finishData: '',
    finishTime: ''
  ...
  const initDetailFunc = () => {
    ...
    console.log('### initDetailFunc initBeginDate: ', initBeginDate);
    console.log('### initDetailFunc initBeginTime: ', initBeginTime);
    console.log('### initDetailFunc initFinishData: ', initFinishData);
    console.log('### initDetailFunc initFinishTime: ', initFinishTime);
  ...
  const addQuestion = () => {
    console.log('#######addQuestion############## : ' , props.voteData.voteSsno)
  ...
  const delQuestion = (voteSsno:number, questionSsno:number) => {
    console.log('#######delQuestion############## voteSsno: ' , voteSsno)
    console.log('#######delQuestion############## questionSsno: ' , questionSsno)
    let qestnList = showQuestionData.value;
  ...
  const editVoteDetail = async () => {
    ...
    console.log('$$$$$$$$$$ editVoteDetailData.value : ', reqData)
    await $fetch<ApiResponse<VoteDetailDataType>>(
      ...
      console.log('editApiCall Method : res : ', res);
