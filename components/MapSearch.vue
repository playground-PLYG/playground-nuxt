...
let markers: any[]
let infowindow: { close: () => void; setContent: (arg0: HTMLDivElement) => void; open: (arg0: any, arg1: any) => void; }
let ps: { keywordSearch: (arg0: string, arg1: (data: place[], status: string, pagination: number) => void, arg2: { x: string; y: string; category_group_code: string; }) => void; }
let map: any
let markerPosition
...
const searchPlaces= () => {
    ...
    ps.keywordSearch( keyword.value, placesSearchCB,  {"x": "126.981727",
    "y": "37.567858", "category_group_code":"FD6"}); 
}

const placesSearchCB = (data:place[], status:string, pagination:number) => {
...
function addMarker(position:Object, idx:number, title:string) {
...
function getListItem(index:number, places:place) {
...
const displayInfowindow = (marker:any, place:place) => {
    ...
    customOverlay.getContent().parentElement.parentElement.style.background = 'transparent'
...
#customoverlay {bottom:85px;border-radius:6px;border: 0;border-bottom:2px solid #ddd;float:left;z-index: 0;}
...
.customoverlayparent {cursor: default; position: absolute; background: transparent; border: 1px solid transparent; z-index: 1; display: block; left: -74px; top: 186px; width: 182px; height: 82px;}
</style>
